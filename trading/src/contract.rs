...
pub trait Trading {
    ...
    fn close_position(e: Env, user: Address, seq: u32, price: Bytes) -> i128;
    ...
    fn modify_collateral(e: Env, user: Address, seq: u32, new_collateral: i128, price: Bytes);
    ...
    /// # Panics
    /// - `TradingError::ContractFrozen` (742) if contract is Frozen
    fn set_triggers(e: Env, user: Address, seq: u32, take_profit: i128, stop_loss: i128);
    ...
    /// - `users` - Position owner addresses (parallel with `seqs`)
    /// - `seqs` - Per-user sequence numbers (parallel with `users`)
    ...
    fn execute(e: Env, caller: Address, market_id: u32, users: Vec<Address>, seqs: Vec<u32>, price: Bytes);
    ...
    /// Returns the position for the given user and sequence number.
    fn get_position(e: Env, user: Address, seq: u32) -> Position;
...
impl Trading for TradingContract {
    ...
    fn cancel_position(e: Env, user: Address, seq: u32) -> i128 {
        storage::extend_instance(&e);
        trading::execute_cancel_position(&e, &user, seq)
    }

    fn close_position(e: Env, user: Address, seq: u32, price: Bytes) -> i128 {
        storage::extend_instance(&e);
        trading::execute_close_position(&e, &user, seq, price)
    }

    fn modify_collateral(e: Env, user: Address, seq: u32, new_collateral: i128, price: Bytes) {
        storage::extend_instance(&e);
        let pv = PriceVerifierClient::new(&e, &storage::get_price_verifier(&e));
        trading::execute_modify_collateral(&e, &user, seq, new_collateral, &pv.verify_price(&price));
    }

    fn set_triggers(e: Env, user: Address, seq: u32, take_profit: i128, stop_loss: i128) {
        storage::extend_instance(&e);
        trading::execute_set_triggers(&e, &user, seq, take_profit, stop_loss);
    }

    fn execute(e: Env, caller: Address, market_id: u32, users: Vec<Address>, seqs: Vec<u32>, price: Bytes) {
        storage::extend_instance(&e);
        let pv = PriceVerifierClient::new(&e, &storage::get_price_verifier(&e));
        trading::execute_trigger(&e, &caller, market_id, users, seqs, &pv.verify_price(&price));
    }
    ...
    fn get_position(e: Env, user: Address, seq: u32) -> Position {
        storage::get_position(&e, &user, seq)
    }
