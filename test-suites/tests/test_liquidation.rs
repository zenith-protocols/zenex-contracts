...
fn trigger_vecs(env: &soroban_sdk::Env, user: &Address, id: u32) -> (soroban_sdk::Vec<Address>, soroban_sdk::Vec<u32>) {
    (svec![env, user.clone()], svec![env, id])
}
...
fn test_liquidation_underwater_position() {
    ...
    let (users, ids) = trigger_vecs(&fixture.env, &user, position_id);
    fixture.trading.execute(&keeper, &FEED_BTC, &users, &ids, &crash_price);
...
fn test_liquidation_healthy_position_rejected() {
    ...
    fixture
        .trading
        .execute(&keeper, &FEED_BTC, &users, &ids, &mild_drop); // should panic #751
}
...
fn test_liquidation_stale_price_rejected() {
    ...
    fixture
        .trading
        .execute(&keeper, &FEED_BTC, &users, &ids, &stale_price); // should panic #749
}
...
fn test_liquidation_after_interest_accrual() {
    ...
    let (users, ids) = trigger_vecs(&fixture.env, &user, position_id);
    fixture.trading.execute(&keeper, &FEED_BTC, &users, &ids, &moderate_drop);
