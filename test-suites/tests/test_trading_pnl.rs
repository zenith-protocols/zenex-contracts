...
fn test_profitable_long_position_small_gain() {
    ...
    assert_eq!(balance_after_open, initial_balance - (1_000 * SCALAR_7));
    ...
    let expected_balance = initial_balance - (1_000 * SCALAR_7) + (1_100 * SCALAR_7);
...
fn test_different_leverage_same_price_movement() {
    ...
    // Close all positions
    let requests1 = svec![&fixture.env, Request { action: RequestType::Close, position: pos1 }];
    let requests2 = svec![&fixture.env, Request { action: RequestType::Close, position: pos2 }];
    let requests3 = svec![&fixture.env, Request { action: RequestType::Close, position: pos3 }];
